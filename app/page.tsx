    ...
    <div className="mt-4">
      ...
      {activeTab === "day" && (
        <>
          ...
          <MetricRow
            label="Volume vs rata-rata 20 hari"
            value={`${(
              ind.volume /
              (ind.avgVolume || 1)
            ).toFixed(2)}x`}
          ...
          {fib.levels.map(
            (lvl) => (
              <MetricRow
                ...
                value={lvl.price.toLocaleString(
                  "id-ID"
                )}
              />
            )
...
function AIAnalysisPanel({
  ...
  async function handleAIAnalysis() {
    ...
    try {
      ...
      const json =
        (await res.json()) as AIAnalysisResponse;

      if (!res.ok) {
        throw new Error(
          json.error ??
            "Gagal mendapatkan analisis AI"
        );
      }
      ...
      setAiError(
        err instanceof Error
          ? err.message
          : "Terjadi kesalahan saat meminta analisis AI."
      );
    } finally {
      setAiLoading(false);
    }
  ...
  return (
    <section className="mt-6 border-t border-neutral-800 pt-6">
        ...
        {!aiResponse &&
          !aiLoading && (
            <button
              ...
              className="shrink-0 rounded-lg bg-neutral-100 px-4 py-2 text-sm font-medium text-neutral-900 transition hover:bg-white"
            >
              🤖 Analisis
              dengan AI
            </button>
      ...
      {aiLoading && (
        <div className="rounded-lg border border-neutral-800 bg-neutral-950 px-4 py-5">
      ...
      {aiResponse &&
        !aiLoading && (
          <div className="rounded-lg border border-neutral-800 bg-neutral-950 p-4">
            ...
            {aiResponse.contents &&
            aiResponse.contents.length >
              0 ? (
              <div className="space-y-4">

                {aiResponse.contents.map(
                  (
                    content,
                    index
                  ) => {
